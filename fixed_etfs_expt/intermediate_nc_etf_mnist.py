...
def train(model, criterion, optimizer, lr_scheduler, trainloader, epochs, epoch_list, one_hot=False, use_cuda=True):
    if model.etf:
        save_dir = os.path.join('mnist_intermediate_expt_%1.3f_%d_etf'%(lr,len(model.hidden_layers_widths)-model.num_etf), 'mse' if one_hot else 'cross_entropy')
    else:
        save_dir = os.path.join('mnist_intermediate_expt_%1.3f_%d'%(lr,len(model.hidden_layers_widths)-model.num_etf), 'mse' if one_hot else 'cross_entropy')
...
if __name__ == "__main__":
    ...
    data = datasets.MNIST(root='mnist', train=True, transform=tx)
