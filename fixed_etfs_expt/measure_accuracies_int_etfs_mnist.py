...
if __name__ == "__main__":
    ...
    traindata = datasets.MNIST(root='mnist', train=True, transform=tx)
    trainloader = DataLoader(traindata, batch_size=batch_size)
    testdata = datasets.MNIST(root='mnist', train=False, transform=tx)
    ...
    for n in num_etfs:
        save_dir = os.path.join('mnist_intermediate_expt_%1.3f_%d_etf'%(lr,layers-n), 'mse' if one_hot else 'cross_entropy')
    ...
    with open('mnist_etfs_accuracy.pkl', 'wb') as f:
