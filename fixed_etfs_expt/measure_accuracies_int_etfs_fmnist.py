...
if __name__ == "__main__":
    ...
    traindata = datasets.FashionMNIST(root='fashion_mnist', train=True, transform=tx)
    trainloader = DataLoader(traindata, batch_size=batch_size)
    testdata = datasets.FashionMNIST(root='fashion_mnist', train=False, transform=tx)
    ...
    for n in num_etfs:
        save_dir = os.path.join('fmnist_intermediate_expt_%1.3f_%d_etf'%(lr,layers-n), 'mse' if one_hot else 'cross_entropy')
        print('Loading %s : %d.pt'%(save_dir,e))
        model = MLP(padded_im_size**2, [1024]*layers, C, use_bias=False, set_etf=etf, num_etf=n)
    ...
    with open('fmnist_etfs_accuracy.pkl', 'wb') as f:
