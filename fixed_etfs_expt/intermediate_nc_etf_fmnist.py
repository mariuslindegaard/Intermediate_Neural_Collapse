...
parser = argparse.ArgumentParser(description='MLP training on MNIST with Simplex ETFs all the way')
...
def train(model, criterion, optimizer, lr_scheduler, trainloader, epochs, epoch_list, one_hot=False, use_cuda=True):
    if model.etf:
        save_dir = os.path.join('fmnist_intermediate_expt_%1.3f_%d_etf'%(lr,len(model.hidden_layers_widths)-model.num_etf), 'mse' if one_hot else 'cross_entropy')
    else:
        save_dir = os.path.join('fmnist_intermediate_expt_%1.3f_%d'%(lr,len(model.hidden_layers_widths)-model.num_etf), 'mse' if one_hot else 'cross_entropy')
...
if __name__ == "__main__":
    ...
    tx = transforms.Compose([transforms.Pad((padded_im_size-im_size)//2), transforms.ToTensor(), transforms.Normalize(0.2860,0.3530)])
    data = datasets.FashionMNIST(root='fashion_mnist', train=True, transform=tx)
    trainloader = DataLoader(data, batch_size=batch_size)

    layers=10
    model = MLP(padded_im_size**2, [1024]*layers, C, use_bias=False, set_etf=args.etf, num_etf=args.num_etf)
    l = 'mse'
    # l = 'cross_entropy'
